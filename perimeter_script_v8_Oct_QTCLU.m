...
%at the faces matrix
[xyz,faces] = read_off('QTCLU_Model_fixed.off');
xyz=xyz';
...
[v i] = max(xyz(:,3));
z_high = v;
meso_z = z_low+(z_high-z_low)/2-0.2;
%meso_z=0.1;

%Define the thickness of the slice of the model 
%where I will pick my perimeter points 
slice_high = meso_z+meso_z/20;
slice_low = meso_z-meso_z/20;
...
for i=1:1:length(x_for_mavlink)
    x_for_mavlink(i) = repdig(x_for_mavlink(i),25058,3);
    y_for_mavlink(i) = repdig(y_for_mavlink(i),410755,4);
end

%Epomeno = x_for_mavlink(1)
...
%The WP
wp_file_create_August_wpfix(x_for_mavlink,y_for_mavlink,10,ptofint)
